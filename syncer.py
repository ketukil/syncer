...
class ColoredFormatter(logging.Formatter):
    ...
    FORMATS = {
        ...
        logging.CRITICAL: Colors.BOLD + Colors.RED + '%(asctime)s - %(levelname)s - %(message)s' + Colors.RESET
    }
...
class ConfigManager:
    ...
    def _create_interactive_config(self) -> None:
        ...
        # Welcome message
        print(f"\n{Colors.BOLD}{Colors.BLUE}=== LAZ File Synchronizer - Initial Configuration ==={Colors.RESET}")
    ...
    def _prompt_for_directories(self) -> None:
        ...
        prompt = f"Enter the path for existing files directory [default: {default_local}]: "
        ...
        prompt = f"Enter the path for download directory [default: {default_download}]: "
        ...
        print(f"{Colors.GREEN}Local directory: {os.path.abspath(local_dir)}{Colors.RESET}")
        print(f"{Colors.GREEN}Download directory: {os.path.abspath(download_dir)}{Colors.RESET}")
    ...
    def _prompt_for_filter_settings(self) -> None:
        ...
        print(f"{Colors.YELLOW}IMPORTANT: If filtering is disabled, no files will be downloaded.{Colors.RESET}")
        ...
        if enable_filter == 'y':
            ...
            case_sensitive = input(f"{Colors.BOLD}{prompt}{Colors.RESET}").strip().lower()
            self.config['FILTER']['case_sensitive'] = 'true' if case_sensitive == 'y' else 'false'
            
            # Show configured filter
            sensitivity = "case-sensitive" if case_sensitive == 'y' else "case-insensitive"
            print(f"{Colors.GREEN}Filter enabled with {sensitivity} pattern: '{self.config['FILTER']['pattern']}'{Colors.RESET}")
        else:
            print(f"{Colors.YELLOW}Filter disabled. You must enable the filter with a pattern to download files.{Colors.RESET}")
            print(f"{Colors.YELLOW}You can enable filtering later using command-line arguments or editing the config file.{Colors.RESET}")
    ...
    def prompt_for_missing_credentials(self) -> None:
        ...
        if not url:
            ...
            url = input(f"{Colors.BOLD}Enter server URL (e.g., http://example.com/files/): {Colors.RESET}")
            self.config['SERVER']['url'] = url
...
class ProgressBar:
    ...
    def _display(self, current: int, total: int, filename: str) -> None:
        ...
        bar = color + '=' * filled_width + Colors.RESET + ' ' * (self.bar_width - filled_width)
...
class ServerParser:
    ...
    def __init__(self, url: str, username: str, password: str, max_retries: int = 3, retry_delay: int = 5):
        """
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if terminate_requested or attempt >= self.max_retries - 1:
                    logger.error(f"Error connecting to server after {attempt+1} attempts: {e}")
                    return []
                
                logger.warning(f"Error connecting to server (attempt {attempt+1}/{self.max_retries}): {e}")
...
class Downloader:
    ...
    def download(self, file_info: FileInfo, local_path: str) -> Tuple[bool, int]:
        ...
        # Start download
        success, bytes_downloaded = self._download_with_retries(file_info, local_path, start_position=downloaded_size)
    ...
    def _download_with_retries(self, file_info: FileInfo, local_path: str, start_position: int = 0) -> Tuple[bool, int]:
        """
        ...
        for attempt in range(self.max_retries):
            if terminate_requested:
                logger.warning(f"Termination requested before download attempt for {file_info.name}")
            ...
            try:
                ...
                if response.status_code == 206:
                    ...
                    total_size = int(total_size_match.group(1)) if total_size_match else 0
                else:
                    # For normal downloads
                    total_size = int(response.headers.get('content-length', 0)) + start_position
                ...
                with open(local_path, mode) as f:
                    for chunk in response.iter_content(chunk_size=self.chunk_size):
                        if terminate_requested:
                            ...
                            f.flush()
                            f.close()
                            logger.warning(f"Download of {file_info.name} interrupted at {format_size(current_size)}")
                            self.progress_bar.finish(success=False)
                        ...
                        if chunk:
                            ...
                            self.progress_bar.update(current_size, total_size, os.path.basename(local_path))
                ...
                if total_size > 0 and final_size < total_size:
                    logger.warning(f"Downloaded file size ({final_size}) is less than expected ({total_size})")
                    return False, bytes_downloaded
                ...
                if terminate_requested:
                    logger.warning(f"Termination requested during download error handling for {file_info.name}")
                    return False, bytes_downloaded
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.info(f"Retrying in {self.retry_delay} seconds from byte {current_size}...")
                    time.sleep(self.retry_delay)
                else:
...
class FileSynchronizer:
    ...
    def __init__(self, config: configparser.ConfigParser):
        ...
        self.progress_update_interval = float(config['DOWNLOAD'].get('progress_update_interval', 1))
    ...
    def sync(self, file_extension: str = '') -> bool:
        ...
        if not filter_enabled:
            ...
            print(message)
            user_response = input(f"{Colors.BOLD}Do you want to enable filtering with pattern '.*' (all files)? (y/n): {Colors.RESET}")
            if user_response.lower() == 'y':
                self.config['FILTER']['enabled'] = 'true'
                self.config['FILTER']['pattern'] = '.*'
                print(f"{Colors.GREEN}Filtering enabled with pattern '.*' to download all files{Colors.RESET}")
            else:
        ...
        files_to_download = self._determine_files_to_download(server_files, local_files, partial_downloads)
        ...
        if not files_to_download:
            if filter_enabled:
                logger.info(f"All matching files are up to date or none match the filter pattern!")
            else:
        ...
        # Confirm download
        confirmation = input(f"{Colors.YELLOW}Continue with download? (y/n): {Colors.RESET}")
    ...
    def _identify_partial_downloads(self, server_files: List[FileInfo]) -> Dict[str, Dict]:
        """
            ...
            if os.path.exists(local_path):
                ...
                if local_size < file.size:
                    partial_downloads[file.name] = {
                        ...
                        'percent_complete': (local_size / file.size) * 100 if file.size > 0 else 0
                    }
    ...
    def _determine_files_to_download(
        ...
        """
        return [file for file in server_files 
    ...
    def _apply_regex_filter(self, files_to_download: List[FileInfo]) -> List[FileInfo]:
        ...
        case_sensitive = self.config.getboolean('FILTER', 'case_sensitive', fallback=False)
        ...
        try:
            ...
            if filtered_out:
                logger.info(f"Filtered out {len(filtered_out)} files using pattern: '{pattern}'")
            logger.info(f"Matched {len(matching_files)} files with pattern: '{pattern}'")
    ...
    def _show_download_summary(
        ...
        if partial_downloads:
            logger.info(f"Including {len(partial_downloads)} partially downloaded files:")
            for name, info in partial_downloads.items():
                ...
                print(f"  {Colors.CYAN}{name}{Colors.RESET}: {color}{info['percent_complete']:.1f}%{Colors.RESET} complete "
        ...
        logger.info(f"Files will be downloaded to: {Colors.BOLD}{os.path.abspath(self.download_dir)}{Colors.RESET}")
        logger.info(f"Press {Colors.BOLD}{Colors.YELLOW}Ctrl+C{Colors.RESET} at any time to gracefully terminate")
    ...
    def _download_files(
        ...
        for i, file in enumerate(files_to_download, 1):
            if terminate_requested:
                logger.warning(f"Termination requested - stopping after {success_count} of {len(files_to_download)} files")
                return False
            ...
            print(f"{Colors.BOLD}File {i} of {len(files_to_download)}{Colors.RESET}: ", end="")
            if existing_size > 0:
                percent = (existing_size / file.size) * 100
                color = Colors.GREEN if percent > 60 else Colors.YELLOW if percent > 30 else Colors.RED
                print(f"Resuming {Colors.CYAN}{file.name}{Colors.RESET} ({color}{percent:.1f}%{Colors.RESET} complete, {format_size(remaining_size)} remaining)")
            else:
                print(f"Downloading {Colors.CYAN}{file.name}{Colors.RESET} ({file.formatted_size})")
    ...
    def _show_final_summary(self) -> None:
        ...
        if terminate_requested:
            status_header = f"{Colors.YELLOW}DOWNLOAD OPERATION TERMINATED BY USER{Colors.RESET}"
        elif self.failed_files:
            status_header = f"{Colors.RED}DOWNLOAD OPERATION COMPLETED WITH ERRORS{Colors.RESET}"
        else:
            status_header = f"{Colors.GREEN}DOWNLOAD OPERATION COMPLETED SUCCESSFULLY{Colors.RESET}"
        
        print(f"{Colors.BOLD}{status_header}{Colors.RESET}\n")
        
        # Show successful downloads
        if self.downloaded_files:
            print(f"{Colors.GREEN}Successfully downloaded files ({len(self.downloaded_files)}):{Colors.RESET}")
        ...
        if self.filtered_files:
            ...
            if len(self.filtered_files) > 10:
                for name in self.filtered_files[:10]:
                    print(f"  {Colors.CYAN}{name}{Colors.RESET}")
                print(f"  {Colors.YELLOW}...and {len(self.filtered_files) - 10} more{Colors.RESET}")
            else:
        ...
        print(f"  Total downloaded: {Colors.BOLD}{format_size(self.total_bytes_downloaded)}{Colors.RESET}")
        print(f"  Time elapsed: {Colors.BOLD}{int(hours)}h {int(minutes)}m {int(seconds)}s{Colors.RESET}")
        
        if elapsed_time > 0 and self.total_bytes_downloaded > 0:
            avg_speed = self.total_bytes_downloaded / elapsed_time
            print(f"  Average download speed: {Colors.BOLD}{format_size(avg_speed)}/s{Colors.RESET}")
            
        print(f"  Files are available in: {Colors.BOLD}{os.path.abspath(self.download_dir)}{Colors.RESET}")
        
        # Show filter information
        filter_enabled = self.config.getboolean('FILTER', 'enabled', fallback=False)
        if filter_enabled:
            pattern = self.config.get('FILTER', 'pattern', fallback='.*')
            sensitivity = "case-sensitive" if self.config.getboolean('FILTER', 'case_sensitive', fallback=False) else "case-insensitive"
        ...
        if terminate_requested and self.failed_files:
            ...
            print(f"{Colors.YELLOW}The program will automatically pick up where it left off.{Colors.RESET}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    Returns:
        Parsed arguments
    """
    parser = argparse.ArgumentParser(description="LAZ File Synchronizer")
    parser.add_argument(
        "-c", "--config", 
    ...
    parser.add_argument(
        "-u", "--url",
        help="Override server URL from config file"
    )
    parser.add_argument(
        "--username",
        help="Override server username from config file"
    )
    parser.add_argument(
        "--password",
    ...
    parser.add_argument(
        "--local-dir",
        help="Override local directory path from config file"
    )
    parser.add_argument(
        "--download-dir",
        help="Override download directory path from config file"
    )
    parser.add_argument(
        "--filter",
...
def main() -> int:
    ...
    print(f"\n{Colors.BOLD}{Colors.BLUE}=== LAZ File Synchronizer ==={Colors.RESET}")
    print(f"{Colors.YELLOW}Press Ctrl+C at any time to gracefully terminate{Colors.RESET}\n")
    ...
    try:
        ...
        if args.download_dir:
            ...
            config_manager.save()
            logger.info(f"Updated download directory in config file: {args.download_dir}")
        ...
        if args.filter:
            ...
            logger.info(f"Using regex filter pattern from command line: '{args.filter}'")
        
        # Validate credentials
        if not validate_server_credentials(config):
            logger.error("Server credentials are incomplete. Please check your configuration.")
            return 1
        ...
        if not test_server_connection(
            ...
            logger.error("Connection test failed. Please check your server credentials.")
            retry = input(f"{Colors.YELLOW}Do you want to update your server credentials? (y/n): {Colors.RESET}")
            if retry.lower() == 'y':
        ...
        if filter_enabled:
            ...
            case_sensitive = config.getboolean('FILTER', 'case_sensitive', fallback=False)
            sensitivity = "case-sensitive" if case_sensitive else "case-insensitive"
            logger.info(f"Regex filtering enabled with {sensitivity} pattern: '{pattern}'")
        else:
            logger.warning(f"Regex filtering is DISABLED. No files will be downloaded unless enabled.")
